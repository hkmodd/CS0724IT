...
Avevo ottenuto l'accesso **root** senza dover fare altro. Ma, ironia della sorte, non ho fatto **assolutamente nulla** con questo accesso root. 

Tornato alla reverse shell caricata in precedenza, è stato qui che ho deciso di affrontare **PwnKit**. Ho sbattuto la testa nel tentativo di **compilare l'exploit** direttamente sulla macchina target. Dopo svariati errori e tentativi falliti, ho abbandonato l'idea della compilazione e sono passato alla soluzione più pratica: usare l'exploit **precompilato**.

---

## 🏆 **Escalation dei Privilegi Alternativa**
Anche se l'accesso **sudo** con l'utente `anne` era sufficiente, ho voluto esplorare altri metodi per ottenere root.
